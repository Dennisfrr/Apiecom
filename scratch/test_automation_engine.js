...
async function testApprovedEditsReachBling() {
  ...
  assert.deepEqual(childUpdate.variacao.produtoPai, { id: 60, cloneInfo: false }); assert.equal(childUpdate.variacao.ordem, 1);
}
...
async function testKitCreatesMissingComponentsFirst() {
  ...
  assert(updatedChildren.every(call => call.body.midia.imagens.imagensURL.length >= 2 && call.body.variacao.produtoPai.id > 0));
